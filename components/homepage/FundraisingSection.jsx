...
function FundraisingSection() {
  ...
  return (
    <section className="bg-[color:var(--paper)] px-5 py-20 sm:px-8 lg:px-10 lg:py-24">
      <div className="mx-auto max-w-7xl">
        {/* Campaign — dark panel */}
        <div className="rounded-[20px] bg-[color:var(--ink)] p-6 text-white sm:p-10 lg:p-12">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              <span className="inline-flex items-center rounded-full border border-white/15 bg-white/5 px-4 py-1.5 text-[0.72rem] font-semibold uppercase tracking-[0.18em] text-white/80">
                Fundraising · Dr. John L. LaNoue Medical Center, Gulu
              </span>

              <h2 className="mt-5 text-[2rem] leading-[1.06] tracking-[-0.03em] text-white sm:text-4xl lg:text-[2.75rem]">
                Healing in His Name
              </h2>
              <p className="mt-3 text-lg font-medium text-[color:var(--brand-accent)]">
                Help build a center of care for communities in need.
              </p>
              <p className="mt-3 max-w-xl text-[0.98rem] leading-relaxed text-white/70">
                Your gift supports construction, equipment, and essential
                services so families can access reliable care closer to home.
              </p>
            </div>

            {/* Progress card */}
            <div className="w-full max-w-md rounded-[14px] border border-white/10 bg-white/5 p-6">
              ...
              <a href="/donate" className="btn btn-accent mt-5">
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-12">
            ...
            <div className="lg:col-span-8">
              <div className="relative overflow-hidden rounded-[16px] border border-white/10 bg-white/5">
                ...
                <div className="pointer-events-none absolute left-4 top-4 inline-flex items-center gap-2 rounded-full bg-black/50 px-4 py-2 text-[0.8rem] text-white/85 backdrop-blur">
                  <Play className="h-3.5 w-3.5 fill-current text-[color:var(--brand-accent)]" />
                  Watch the story
                </div>
                ...
                <div className="flex flex-col gap-4 border-t border-white/10 bg-black/30 p-5 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <a
                      href="/donate"
                      className="inline-flex items-center justify-center gap-2 rounded-full bg-[color:var(--paper)] px-5 py-2 text-sm font-semibold text-[color:var(--ink)] transition hover:-translate-y-0.5"
                    >
            ...
            <aside className="lg:col-span-4">
              <div className="h-full rounded-[16px] border border-white/10 bg-white/5 p-6">
                ...
                <div className="mt-6 space-y-3">
                  {pillars.map((item) => (
                    ...
                    >
                      <item.icon
                        className={`mt-0.5 h-5 w-5 flex-none ${item.tone}`}
                      />
                      <div>
        ...
        {/* Project updates & field videos — WHITE */}
        <div id="updates" className="mt-16">
          <div className="flex items-end justify-between gap-4">
            ...
            <a
              href="/donate"
              className="hidden items-center gap-2 rounded-full border border-[color:var(--line)] bg-[color:var(--paper)] px-4 py-2 text-sm font-medium text-[color:var(--ink)] transition hover:border-[color:var(--brand-primary)] sm:inline-flex"
            >
              Keep it moving{" "}
              <HeartHandshake className="h-4 w-4 text-[color:var(--brand-primary-700)]" />
            </a>
          </div>
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {videos.map((v, i) => {
              ...
              return (
                <article
                  key={i}
                  className="overflow-hidden rounded-[14px] border border-[color:var(--line)] bg-[color:var(--paper)] transition hover:-translate-y-1 hover:shadow-[0_28px_65px_-45px_rgba(12,34,51,0.5)]"
                >
                  <div className="relative m-2.5 aspect-video overflow-hidden rounded-[12px]">
                    <iframe
                      className="absolute inset-0 h-full w-full"
                      ...
                      allowFullScreen
                    />
                    <div className="pointer-events-none absolute left-3 top-3 inline-flex items-center gap-2 rounded-full bg-white/90 px-3 py-1.5 text-xs font-medium text-[color:var(--ink)] backdrop-blur">
                      <Icon className="h-3.5 w-3.5 text-[color:var(--brand-primary-700)]" />
                      {v.tag}
                    </div>
                  </div>

                  <div className="px-5 pb-5 pt-1">
                    <h4 className="text-base font-medium leading-snug text-[color:var(--ink)]">
                      {v.title}
                    </h4>
                    <p className="mt-2 text-[0.85rem] text-[color:var(--muted)]">
                      {v.note}
                    </p>
                    <div className="mt-4 flex items-center justify-between">
                      <a href="/donate" className="btn btn-primary btn-sm">
